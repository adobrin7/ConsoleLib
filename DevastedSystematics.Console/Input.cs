...
public static class Input
{
    ...
    public static bool TryRead(string validationRegex, out string? input) => 
        TryRead(validationRegex, "", out input);

    public static bool TryRead(
        string validationRegex, 
        string inputPrecedingMessage, 
        out string? input)
    {
        try
        {
    ...
    /// <exception cref="ArgumentOutOfRangeException">Throws if attempts count <= 0.</exception>
    /// <exception cref="FormatException">Throws when input doesn't match pattern and no attempts remaining.</exception>
    public static string ReadUntilValidOrOutOfAttempts(
        string validationRegex, 
        string inputPrecedingMessage, 
        int attemptsCount) => 
    ...
    public static bool TryReadUntilValidOrOutOfAttempts(
        string validationRegex, 
        int attemptsCount, 
        out string? input) => 
    ...
    public static bool TryReadUntilValidOrOutOfAttempts(
        ...
        int attemptsCount, 
        out string? input) => 
        TryReadUntilValidOrOutOfAttempts(
            validationRegex, 
            inputPrecedingMessage, 
            attemptsCount, 
            "", 
            out input);
        ...
        out string? input)
    {
        try
        {
            input = ReadUntilValidOrOutOfAttempts(
                validationRegex, 
                inputPrecedingMessage, 
                attemptsCount, 
                invalidInputMessage);
    ...
    /// <param name="validationRegex">Pattern.</param>
    /// <returns>Validated input.</returns>
    public static string ReadUntilValid(string validationRegex) => 
        ReadUntilValid(validationRegex, "");
    ...
    /// <param name="invalidInputMessage">Message to show after validation failed message.</param>
    /// <returns>Validated input.</returns>
    public static string ReadUntilValid(
        string validationRegex, 
        string inputPrecedingMessage, 
        string invalidInputMessage)
    {
